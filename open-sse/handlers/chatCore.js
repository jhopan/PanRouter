...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking }) {
  ...
  // reject legacy payloads that omit it with HTTP 400. Provider-scoped via quirks
  // (shouldDefaultClaudeToolType): only gateways that declare requireClaudeToolType get the
  // explicit type. Applying it unconditionally breaks Claude-format endpoints that only accept
  // the legacy typeless tool shape — DeepSeek's Anthropic-compatible endpoint 400s with
  // "unknown variant `custom`" (#3905), and AgentRouter's /v1/messages relay rejects the
  // synthetic `type: "custom"` the same way (agentrouter declares no such quirk).
